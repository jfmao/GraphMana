...
\noindent\textbf{Supplementary Figure 4.~FAST PATH mechanism.}~FAST PATH access strategy in detail. (\textbf{a})~Internal structure of a Variant node showing the two data stores: left, \texttt{gt\_packed} byte array (2 bits per sample, 801 bytes for 3,202 samples) that grows linearly with $N$; right, pre-computed population arrays (\texttt{pop\_ids[]}, \texttt{ac[]}, \texttt{an[]}, \texttt{af[]}) containing only $K$ values where $K$ is the number of populations---constant in size regardless of $N$. (\textbf{b})~Tools partitioned by data requirement: TreeMix, dadi, fastsimcoal2, and bedtools/allele frequency queries need only population summaries and are served by FAST PATH; PLINK, EIGENSTRAT, VCF, and Beagle/STRUCTURE need individual genotypes and use FULL PATH. (\textbf{c})~Scaling: the FAST PATH advantage grows with $N/K$, from 20$\times$ at 100 samples to 5{,}000$\times$ at 50{,}000 samples. Real benchmark on chr22 (1.07M variants, 3,202 samples): TreeMix export completes in 46~s vs PLINK in 75~s (1.6$\times$ speedup). At chromosome scale the advantage is modest because both paths are dominated by the Neo4j query overhead; at whole-genome scale (85M variants) the per-variant $O(K)$ vs $O(N)$ difference dominates. (\textbf{d})~Workflow comparison for TreeMix export: without FAST PATH, 3,202 genotypes must be unpacked per variant then counted (75~s total); with FAST PATH, the \texttt{gt\_packed} array is never touched and allele counts are read directly from \texttt{ac[]} and \texttt{an[]} (46~s total).
...
\caption*{\textbf{Supplementary Figure 3.~VCF-to-graph mapping in detail.}}
\end{figure}
...
\caption*{\textbf{Supplementary Figure 4.~FAST PATH mechanism.}}
\end{figure}
...
\caption*{\textbf{Supplementary Figure 5.~FULL PATH mechanism.}}
\end{figure}
...
\caption*{\textbf{Supplementary Figure 8.~How VCF data is stored in GraphMana's graph database.}}
\end{figure}
